...
int calc_day_num(int year, int month, int day) {
  ...
  // add something to handle a Leap Year (aka make february 1 day longer and 366
  // instead of 365) leap years include 2008, 2012, 2016, 2020
  ...
  // months agter february has to be given +1 day value because there is now 366
  // days and not 365
}
...
//-----------------------------------------------------------------------------------------
void analysis2(const std::vector<Measurement>& data,
  ...
  // I thought that was excessive. Originally I did use a map, I learned what to
  // write, but it felt too clunky. Like it served no purpose in this specific
  // dataset, since its pretty straight forward. I thought a vector would work
  // fine So i went back and re-wrote it simpler(oct 30)
  ...
  for (const auto& m : data) {
    ...
    if (m.getYear() < 2005 ||
        (m.getYear() == 2005 &&
         (m.getMonth() < 2 || (m.getMonth() == 2 && m.getDay() < 1))))
      continue;
    if (m.getYear() > 2023 ||
        (m.getYear() == 2023 &&
         (m.getMonth() > 2 || (m.getMonth() == 2 && m.getDay() > 1))))
  ...
  TH1F* AvgTemp = new TH1F("AvgTemp",
                           "Mean Daily Temperature (February 1 2005 - February "
                           "1 2023);Day of Year;Temperature [C]",
                           nDays, 0.5, nDays + 0.5);
  ...
  for (int day = 0; day < nDays; ++day) {
    if (tempsonday[day].empty())
      continue;  // check if day, and days temp, exists bc of leap year

    const auto& temps =
        tempsonday[day];  // temperatures for every instance of the same date
  ...
  AvgTemp->GetXaxis()->LabelsOption(
      "h");  // i wanted the words to be flat not tall

  // 6) draw the histogram and then save it //this is from the slides basically
  TCanvas* analysis2canvas =
      new TCanvas("Analysis2Canvas", "Mean Daily Temperature", 1000, 600);
  AvgTemp->Draw("HIST");  // make line first
  AvgTemp->SetLineColorAlpha(
      kViolet - 2, 0.5);    // slightly transparent to improve readability
  AvgTemp->Draw("E SAME");  // make error bars
  analysis2canvas->Update();

  // Determine output directory
  // asked chat gpt for assistance because i was so confused why there was no
  // saved files outputted i tested it so many times and recompiled like a
  // billion times and got NOTHING finally realized that the real issue was my
  // header file named incorrectly... so my code never ran and it wasnt the
  // output that was the problem!! oops!
  ...
  // Save path for latex use (not sure if i need that but chat said it was a
  // good idea to have that as well, so I saved it as a png)
  std::filesystem::path pdf_file = outDir / "MeanDailyTemperature.pdf";
  analysis2canvas->SaveAs(pdf_file.c_str());

  TFile outFile(
      output_filename.c_str(),
      "UPDATE");  // changed to UPDATE because we want 1 file with all analyses
  AvgTemp->Write();
  outFile.Close();

  std::cout
      << "Saved Histogram for Analysis 2 as pdf (for latex) and to ROOT file"
      << '\n';
}
