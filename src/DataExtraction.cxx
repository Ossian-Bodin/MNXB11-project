...
std::vector<Measurement> read_measurements(const std::string& filename){ //constructor initialization 
    ...
    while(file >> year >> dash >> month >> dash >> day >> hour >> colon >> minute >> colon >> second >> temp >> quality){
...
void persist_measurements(const std::vector<Measurement>& measurements, const std::string& output_filename){ //constructor initialization
