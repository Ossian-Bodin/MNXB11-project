...
IceCreamData readIceCreamCSV() {
  ...
  const std::string& filename =
      "datasets/ice_cream.csv";  // hardcoded but fine for now. We only have
                                 // this one dataset
  ...
  while (std::getline(file, line))  // loop to read lines
  {
    ...
    if (std::getline(ss, dateStr, ',') &&
        std::getline(
            ss,
            salesStr))  // push ','-separated items to dates and sales vectors
    {
      data.dates.push_back(dateStr);
      data.sales.push_back(
          std::stod(salesStr));  // stod::convert string to double
    }
  }
...
TemperatureData computeMonthlyTemp(const std::vector<Measurement>& measurements,
                                   int startyear, int stopyear) {
  TemperatureData
      tempData;  // create empty struct to store year, month, tempAvg

  // for each key (month, year) associate a vector containing all (hourly)
  // temperature readings that (month, year)
  std::map<std::pair<int, int>, std::vector<double>>
      tempMonthBox;  // to be averaged
  ...
  for (const auto& m : measurements) {
    ...
    if (year >= startyear && year <= stopyear) {
      tempMonthBox[{year, month}].push_back(
          temp);  // fill the map with temps (for averaging)
    }
  }
  ...
  for (int year = startyear; year <= stopyear; ++year) {
    for (int month = 1; month <= 12; ++month) {
      ...
      auto it = tempMonthBox.find(
          key);  // iterator over key (GPT showed me iterators)
...
void plotTempVsSales(const std::vector<Measurement>& measurements,
  ...
  IceCreamData iceCream = filterIceCreamByYear(
      iceCreamFull, startyear,
      stopyear);  // fill iceCream struct with data between startyear - stopyear

  // check that data sets have equal number of months read.
  size_t n =
      std::min(temp.avgTemps.size(),
               iceCream.sales.size());  // collect smallest data set length
  ...
  std::cout << "After resize: " << temp.avgTemps.size()
            << " temperature entries vs. " << iceCream.sales.size()
            << " sales entries.\n\n";

  std::cout << std::left << std::setw(15) << "IceCreamDate" << std::setw(12)
            << "Sales" << std::setw(12) << "TempDate" << std::setw(10)
            << "AvgTemp"
            << "\n";
  std::cout
      << "-------------------------------------------------------------\n";
  ...
  for (int N = 0; N < 10; ++N) {
    ...
    std::cout << std::left << std::setw(15) << iceDate << std::setw(12) << sale
              << std::setw(12) << ym.str() << std::setw(10) << avg << "\n";
  }
  ...
  TH1D* tempHist =
      new TH1D("tempHist", "Temperature VS Ice Cream Sales", nBins, 0, nBins);
  ...
  auto canv =
      new TCanvas("canv", "Monthly Temperature VS Ice cream sales", 1200, 600);
  ...
  // set axes
  std::string Xtitle = "Months from " + std::to_string(startyear) + " to " +
                       std::to_string(stopyear);
  ...
  TGaxis* axis = new TGaxis(xminPad, yZero, xminPad, ymaxPad, 0, rightMax, 510,
                            "+L");  // (?)
